...
\subsection{Parameter sweep examples}
\begin{lstlisting}[language=bash]
# Vary squeezing
python3 clacod_heat1d_qutip.py --r-target 0.5 --r-prime 0.001

# Vary beta
python3 clacod_heat1d_qutip.py --beta 0.8

# Use half convention for comparison
python3 clacod_heat1d_qutip.py --position-convention half

# CHO-style plots
python3 clacod_heat1d_qutip.py \
  --trajectory-steps 80 \
  --component-plot results_bosonic/clacod_CHOstyle_all4.png \
...
\begin{enumerate}[itemsep=3pt]
  ...
  \item \textbf{The squeezed-Fock method's value is in its physical realizability.}
        ...
        and post-select on $S(r)|0\rangle$.  The position-eigenbasis method
        requires arbitrary Fock-state loading, which is itself a nontrivial
        quantum task.
\end{enumerate}
