...
\title{1D Heat Equation with Hybrid CV-DV LCHS}
\date{\today}
...
A direct gate test in this repository confirms the backend Heisenberg-map shifts
\begin{equation}
\Delta x = 2a_0\cos\phi,\qquad \Delta p = 2a_0\sin\phi
\end{equation}
for the same convention.
A historical failure mode was assuming this branch incorrectly from notation only.

In this implementation, we therefore treat phase as an explicit model parameter and provide a built-in calibration test:
\begin{equation}
...
\begin{itemize}
\item \texttt{heat1d\_lchs\_cv\_dv.py}: single-run circuit + metrics + phase calibration.
\item \texttt{heat1d\_lchs\_sensitivity.py}: theorem-constrained deterministic sensitivity/optimization.
\item \texttt{tests/test\_gate\_math.py}: direct gate-math verification (generator/decomposition/tape checks).
\end{itemize}

\section*{9. Reproducible Command Protocol}
Step 0: gate-math sanity (fast fail):
\begin{verbatim}
pytest -q tests/test_gate_math.py
\end{verbatim}

Step 1: phase calibration with baseline export:
\begin{verbatim}
python heat1d_lchs_cv_dv.py --calibrate-phase \
  --output-json results/baseline_phase.json
\end{verbatim}
Use the selected \texttt{disp\_phase} from this run in all subsequent steps.

Step 2: sensitivity sweep with fixed phase:
\begin{verbatim}
...
Step 3: Trotter convergence at fixed $(r,r',\beta,\phi)$:
\begin{verbatim}
