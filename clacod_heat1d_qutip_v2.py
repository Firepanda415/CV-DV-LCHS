...
COEFF_METHODS = ("explicit_overlap", "gh_comp", "legacy_gh", "fitted_single_t", "fitted_multi_t")
...
class HeatLCHSConfig:
    ...
    fit_times: int = 8  # number of time points for fitted_multi_t
    fitted_maxiter: int = 2000  # max L-BFGS-B iterations for fitted methods
...
def _build_lchs_map_at_t(
    ...
    """Build the LCHS map K(t) for a given coefficient vector and time."""
    coeffs_full = np.zeros(n_fock, dtype=complex)
    n_coeff = len(coeffs)
    coeffs_full[:n_coeff] = coeffs
    psi_seed = Qobj(coeffs_full.reshape((-1, 1))).unit()
    psi_osc = (squeeze(n_fock, r_prime) * psi_seed).unit()
    phi_post = (squeeze(n_fock, r_target) * basis(n_fock, 0)).unit()
    a = destroy(n_fock)
    if position_convention == "half":
        x_hat = (a + a.dag()) / 2.0
    else:
        x_hat = (a + a.dag()) / np.sqrt(2.0)
    dim_dv = a_matrix.shape[0]
    a_qobj = Qobj(a_matrix, dims=[[dim_dv], [dim_dv]])
    u_joint = (-1.0j * t * tensor(x_hat, a_qobj)).expm()
    post_bra = tensor(phi_post.dag(), qeye(dim_dv))
    embed = tensor(psi_osc, qeye(dim_dv))
    return np.asarray((post_bra * u_joint * embed).full(), dtype=complex)


def _fitted_objective_single_t(
    coeffs_re_im: np.ndarray,
    r_target: float,
    r_prime: float,
    n_fock: int,
    n_coeff: int,
    ...
    """Cost function for fitted_single_t: minimize map error at one time."""
    c = coeffs_re_im[:n_coeff] + 1.0j * coeffs_re_im[n_coeff:]
    norm = np.linalg.norm(c)
    if norm < 1e-15:
        return 1e10
    c = c / norm
    k_map = _build_lchs_map_at_t(c, r_target, r_prime, n_fock, t, position_convention, a_matrix)
    ref_map = expm(-a_matrix * t)
    denom = np.vdot(ref_map, ref_map)
    if np.isclose(denom, 0.0):
        return 1e10
    scale = np.vdot(ref_map, k_map) / denom
    return float(np.linalg.norm(k_map - scale * ref_map) / max(np.linalg.norm(scale * ref_map), 1e-15))


def _fitted_objective_multi_t(
    coeffs_re_im: np.ndarray,
    r_target: float,
    r_prime: float,
    n_fock: int,
    ...
    """Cost function for fitted_multi_t: minimize mean map error across times."""
    c = coeffs_re_im[:n_coeff] + 1.0j * coeffs_re_im[n_coeff:]
    norm = np.linalg.norm(c)
    if norm < 1e-15:
        return 1e10
    ...
    for t in time_points:
        ...
        if np.isclose(denom, 0.0):
            continue
        scale = np.vdot(ref_map, k_map) / denom
        err = float(np.linalg.norm(k_map - scale * ref_map) / max(np.linalg.norm(scale * ref_map), 1e-15))
        total_err += err
    return total_err / max(len(time_points), 1)


def lchs_coefficients_fitted(
    r_target: float,
    r_prime: float,
    beta: float,
    n_fock: int,
    ...
    """Numerically optimized coefficients via L-BFGS-B.

    Parameters
    ----------
    mode : str
        "single_t" — optimize for one time point (total_time).
        "multi_t"  — optimize across fit_times evenly spaced points in (0, total_time].
    fit_times : int
        Number of time points for multi_t mode.
    maxiter : int
        Maximum optimizer iterations.

    Notes
    -----
    - single_t can interpolate a small matrix almost exactly but overfits to one t.
    - multi_t produces coefficients that generalize across times, acting more like
      a true kernel realization.  Expect higher per-point error but much better
      generalization.
    """
    from scipy.optimize import minimize

    a_matrix = heat_matrix(alpha, h_grid)

    # Seed from analytical formula
    seed = lchs_coefficients_explicit_overlap(
        r_target=r_target,
        r_prime=r_prime,
        beta=beta,
        n_fock=n_coeff,
    ...
    x0 = np.concatenate([seed.real, seed.imag])

    if mode == "single_t":
        result = minimize(
            _fitted_objective_single_t,
            x0,
            args=(r_target, r_prime, n_fock, n_coeff, total_time, position_convention, a_matrix),
            method="L-BFGS-B",
            options={"maxiter": maxiter, "ftol": 1e-15},
        )
    elif mode == "multi_t":
        time_points = np.linspace(total_time / fit_times, total_time, fit_times)
        result = minimize(
            _fitted_objective_multi_t,
            x0,
            args=(r_target, r_prime, n_fock, n_coeff, time_points, position_convention, a_matrix),
            method="L-BFGS-B",
            options={"maxiter": maxiter, "ftol": 1e-15},
        )
    else:
    ...
    c = result.x[:n_coeff] + 1.0j * result.x[n_coeff:]
    norm = np.linalg.norm(c)
    if np.isclose(norm, 0.0):
        raise RuntimeError("Fitted coefficient vector has zero norm.")
    return c / norm
...
def generalization_diagnostic(
    ...
    """Sweep time from 0 to 2*total_time and report map error at each point.

    Useful for detecting overfitting of fitted_single_t coefficients.
    """
    a_mat = heat_matrix(cfg.alpha, cfg.h_grid)
    a_qobj = Qobj(a_mat, dims=[[4], [4]])
    x_hat = position_operator(cfg.n_fock, cfg.position_convention)

    rows: List[Dict[str, float]] = []
    t_max = 2.0 * cfg.total_time
    for i in range(n_times + 1):
        t = (i / n_times) * t_max
        if t < 1e-14:
            rows.append({"time": 0.0, "map_error": 0.0})
            continue
        u_joint = (-1.0j * t * tensor(x_hat, a_qobj)).expm()
        post_bra = tensor(phi_post.dag(), qeye(4))
        embed = tensor(psi_osc, qeye(4))
        k_map = np.asarray((post_bra * u_joint * embed).full(), dtype=complex)
        ref_map = expm(-a_mat * t)
        denom = np.vdot(ref_map, ref_map)
        if np.isclose(denom, 0.0):
            rows.append({"time": float(t), "map_error": float("nan")})
            continue
        scale = np.vdot(ref_map, k_map) / denom
        err = float(np.linalg.norm(k_map - scale * ref_map) / max(np.linalg.norm(scale * ref_map), 1e-15))
        rows.append({"time": float(t), "map_error": err})
    return rows
...
def lchs_coefficients(
    ...
    **fitted_kwargs,
    ...
    if method in ("fitted_single_t", "fitted_multi_t"):
        mode = method.replace("fitted_", "")
        return lchs_coefficients_fitted(
            r_target=r_target,
            r_prime=r_prime,
            beta=beta,
            n_fock=fitted_kwargs.get("fitted_n_fock", 32),
            n_coeff=n_fock,
            mode=mode,
            total_time=fitted_kwargs.get("total_time", 1.0),
            alpha=fitted_kwargs.get("alpha", 1.0),
            h_grid=fitted_kwargs.get("h_grid", 1.0),
            position_convention=fitted_kwargs.get("position_convention", "sqrt2"),
            fit_times=fitted_kwargs.get("fit_times", 8),
            maxiter=fitted_kwargs.get("fitted_maxiter", 2000),
        )
...
def prepare_lchs_states(cfg: HeatLCHSConfig) -> Tuple[Qobj, Qobj, np.ndarray]:
    ...
    coeffs = lchs_coefficients(
        ...
        fitted_n_fock=cfg.n_fock,
        total_time=cfg.total_time,
        alpha=cfg.alpha,
        h_grid=cfg.h_grid,
        position_convention=cfg.position_convention,
        fit_times=cfg.fit_times,
        fitted_maxiter=cfg.fitted_maxiter,
    )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--fit-times", type=int, default=8, help="Time points for fitted_multi_t.")
    p.add_argument("--fitted-maxiter", type=int, default=2000, help="Max L-BFGS-B iterations for fitted methods.")
    p.add_argument(
        "--generalization-n",
        type=int,
        default=0,
        help="If >0, sweep map error over N time points in [0, 2*total_time].",
    )
...
def main() -> None:
    ...
    cfg = HeatLCHSConfig(
        ...
        fitted_maxiter=args.fitted_maxiter,
    )
