...
### 15. DV ancilla PREP breakdown and MPS state-preparation infidelity (tab:dv_circuit_compare one-time rows, DV-comparison prep sentences)
- **Artifacts**: `dv_prep_fidelity.json` (+ `.meta.json`; bond profile of the exact 9-qubit kernel MPS, deterministic gate histogram and depth of the layered PREP circuit, and the state-fidelity distribution of the prepared state against the exact normalized magnitude profile for num_layers = 1..4, 50 conversion draws each).
- **Generation** (repo root):
  ```bash
  /Users/zhen002/miniconda3/envs/cvdv/bin/python -W ignore \
      make_dv_prep_fidelity.py               # cvdv environment (scikit_tt)
  ```
- **Validation gates** (in-script): the exact amplitude vector must produce the full-rank bond profile [1, 2, 4, 8, 16, 16, 8, 4, 2, 1] and an untruncated-MPS fidelity of 1 to 1e-10; the rebuilt two-layer PREP histogram {u: 130, cx: 48} and depth 71 must equal the archived `kernel_prep_operation_counts` / `kernel_prep_depth` in all four circuit summaries (M = 4 trotter100 and the M = 8/16/32 revision summaries); gate counts and depth must be identical across all draws per layer count.
- **Results**: the upstream MPS-to-circuit conversion (the iterative-disentangling scheme of Ran, PRA 101, 032310 (2020)) completes each extracted isometry with unseeded random unitary columns, which do not affect the num_layers = 1 state (deterministic infidelity 2.654e-3, min = max over 50 draws) but perturb num_layers >= 2, so the archived compiled circuits carry one unrecoverable draw. At the compiled num_layers = 2 (130 1Q + 48 CX, the same counts as unPREP), the state-preparation infidelity over 50 draws is mean 1.513e-3, standard deviation 1.342e-4 (`state_infidelity_std` in the JSON), min 1.159e-3, max 1.934e-3, quoted in the paper as (1.51 +/- 0.13)e-3. Deeper conversions improve accuracy at linear gate cost: 195/72 gates reach mean 7.7e-4 and 260/96 reach mean 4.8e-4. As a numerically exact reference alternative, compiling the same 512-node profile with qiskit StatePreparation (the isometry decomposition of Iten et al., PRA 93, 032318 (2016)) costs 511 1Q + 502 CX at depth 1005 with fidelity 1, and the CX count is unchanged under opt-2 transpilation (`exact_isometry_loading` in the JSON).
- **Paper mapping**: the four one-time rows of `tab:dv_circuit_compare` (ancilla PREP 130 1Q / 48 CX, kernel phase layer 511 1Q / 510 CX; unPREP equals PREP, and the remaining single 1Q gate of the 772/606 offset is the input-loading X) come from `kernel_prep_operation_counts` and the 511/510 diagonal delta of Section 14, the exact isometry PREP rows (511 1Q / 502 CX, cited to Iten et al.) come from `exact_isometry_loading`, the MPS-to-circuit conversion is cited to Ran (PRA 101, 032310 (2020)) as `ran2020mps`, and the DV-comparison sentence "state-preparation infidelity of about 1.5e-3" quotes the num_layers = 2 mean of this artifact.

### 16. DV fixed-scale map error and LCU sampling overhead (Section 7.5 comparison additions)
- **Artifacts**: `dv_fixed_scale.csv` (+ `dv_fixed_scale.meta.json`).
- **Generation** (repo root, base environment): `python3 make_dv_fixed_scale.py`.
- **Validation gates** (in-script, before writing): the full beta scan of `make_dv_extension.dv_case` is re-run for all ten instances and must reproduce the published M = 4 rows digit-for-digit (beta and M_DV exact, 1 - F_DV at four significant digits, ||c||_1 at four decimals) and match every row of `dv_extension.csv` and `dv_extension_m32.csv` to relative tolerance 1e-10.
- **What it is**: the two DV quantities missing from `tab:dv_resource_compare`, computed at the published beta_best and sizing (eta = 1, eps = 0.1, T = 1) with exact branch propagators. The fixed-scale map error eps_F_DV = ||K_DV - exp(-AT)||_F / ||exp(-AT)||_F uses the unscaled quadrature map K_DV = sum_j c_j exp(-iT(k_j L + H)) with no fitted rescaling, mirroring the CV fixed-scale convention (the physical LCU block is K_DV/||c||_1 with ||c||_1 known a priori). p_DV_basis1 = ||K_DV u0||^2 / ||c||_1^2 is the all-zero-ancilla acceptance probability of the LCU block for the benchmark input u0 = basis index 1.
- **Results**: eps_F_DV = 12.82 / 16.48 / 4.38 percent for the M = 4 Dirichlet/Neumann/periodic rows, 8.14 / 7.92 / 7.98 percent for heat M = 8/16/32, 8.10 / 7.94 / 7.99 percent for advection-diffusion M = 8/16/32, and 9.87 percent for the 2D case, versus the CV route's 0.47-1.48 percent (2D: 7.40 percent), so the CV route has the smaller fixed-scale error in all ten instances at the prescribed eps = 0.1 sizing. p_DV_basis1 = 23.1-26.2 percent (1/p = 3.8-4.3 attempts) for every 1D instance and 2.00 percent (49.9 attempts) for the 2D case, so the DV route has the smaller benchmark-input sampling overhead in 1D (CV: 6.07-9.55 attempts at M = 4) while both routes collapse together for 2D, where the overhead is set by the decayed solution norm. The p_DV_min/p_DV_max columns give the acceptance-probability range over all computational-basis inputs (e.g. Dirichlet M = 4: 11.71-25.48 percent; 2D: 1.00-3.82 percent); for the periodic instance the probability is input independent (24.33 percent for every basis input, matching the input-independent CV value 15.36 percent from the finite_map artifact), and the paper's Table 7 quotes benchmark value plus range for both routes.
- **Paper mapping**: the revised Section 7.5 sentences reporting the DV fixed-scale error and the two-sided sampling comparison, and the ||c||_1 definition added to the `tab:dv_resource_compare` caption. Caveat recorded in the meta sidecar: the published beta_best was selected on conditional fidelity, and for the Neumann row the eps_F-optimal scan point would be beta = 0.70 at 7.87 percent, so the comparison text states the beta selection criterion explicitly.
