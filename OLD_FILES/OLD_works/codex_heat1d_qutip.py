...
class HeatLCHSConfig:
    ...
    position_convention: str = "half"  # "half" -> (a+a^dag)/2, "sqrt2" -> (a+a^dag)/sqrt(2)
    # Empirical scale calibration for the x-hat convention used here.
...
def position_operator(n_fock: int, convention: str, scale: float) -> Qobj:
    a = destroy(n_fock)
    if convention == "half":
        x_hat = (a + a.dag()) / 2.0
...
def effective_lchs_map(cfg: HeatLCHSConfig, psi_osc: Qobj, phi_post: Qobj) -> np.ndarray:
    ...
    x_hat = position_operator(cfg.n_fock, cfg.position_convention, cfg.position_scale)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--n-fock", type=int, default=128)
    p.add_argument("--n-coeff", type=int, default=128)
...
def main() -> None:
    ...
    print(
        "Operator config: "
        f"position_convention={cfg.position_convention}, position_scale={cfg.position_scale}"
    )
